...
`expcap` 不和 Codex / Claude Code 的个人记忆竞争。它专注于项目级、团队级、公司级的工程经验资产治理：可共享、可审阅、可交割，不绑定某一个人的模型账号。

它不是一个不断自动压缩历史的长期记忆池。原始 trace 是一等证据；总结先生成 candidate，而不是直接成为真理；只有经过作用域约束、证据支撑和审核的内容才应晋升为 asset。
