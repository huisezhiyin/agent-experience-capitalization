...
面向 coding agent 的项目级/团队级经验治理层。

`expcap` 把有效的 agent 工作沉淀成证据驱动、可治理的工程资产。这些资产属于项目和团队，而不是某个用户、某台机器或某个模型账号。
...
个人记忆解决的是“一个 agent 记住一个用户”。团队需要的是另一种东西：能跟随代码库流转、能回到原始证据、并能持续治理的工程经验。

`expcap` 更准确的定位是：面向 coding agent 的**容错经验治理层**。

它关注：

- 项目级规则、模式、上下文和检查清单；
- 可审阅、可共享、可交割的团队资产；
- 任务开始前自动获取经验，任务结束后生成受治理的候选经验；
- 本地优先运行，同时保留云端共享后端的演进路径。

它不是一个不断自动压缩历史的长期记忆池。原始 trace 是一等证据；LLM 总结先生成 candidate，而不是直接生成真理；长期资产必须带作用域、证据链和反馈治理。
...
`expcap` 的设计目标不是不断把旧记忆压缩成新记忆，而是降低连续 consolidation 带来的失真风险。

核心规则：

- 原始 trace 不被抽象记忆替代。
- consolidation 只生成 candidate，不生成真理。
- promote 必须同时满足证据、审核和作用域约束。
- retrieval 返回的是带来源的候选，不是命令。
- 被激活资产必须接受反馈、降权、隔离或淘汰。
- 默认阻止不同任务类型、不同项目层级之间的污染。
- 抽象经验必须始终能回链到可恢复的原始证据。

一句话定位：

> `expcap` 不是 memory-consolidation pile，而是面向 coding agent 的容错经验治理层。
...
- `knowledge_scope`：当前实现主要是 `project` 或 `cross-project`；未来 `team` 和 `organization` 级别应有更严格的晋升门槛。
...
- `personal / local prior`：个人偏好、工作习惯、不要重复说的上下文，不应默认污染团队知识。
- `project asset`：默认主形态，承载仓库局部规则、架构决策、模块经验和常见坑。
- `team asset`：跨多个项目验证过的团队经验，必须带 owner、evidence 和 review。
- `organization asset`：公司级稳定工程知识，需要版本、有效期和废弃机制。
...
更准确的说法不是“三层存储”，而是四类职责：

- `Evidence Store`：原始证据真源，保存 raw trace、task input、tool calls、diff、测试结果、错误日志、activation view 和用户反馈。本地模式下主要是 JSON / JSONL / logs；团队模式应演进到 object storage。
- `Curated Markdown Memory`：稳定可读知识层，保存 `PROJECT_PROMPT.md`、`AGENTS.md`、`AGENTS.expcap.md`、curated memory docs 和经审核的 codemap/docs 材料。
- `Governance DB`：治理状态层，负责 traces/episodes/candidates/assets/activations 的生命周期、状态、关系、热度、review queue 和晋升/废弃历史。当前本地实现是 SQLite，团队模式应演进到 Postgres / Cloud SQL。
- `Semantic Retrieval Layer`：语义召回层，保存 embedding 与 metadata filter，用于“找得到”相关经验。当前实现是 Milvus Lite / 托管 Milvus。

一句话边界：

- Milvus 负责召回，不负责真源。
- SQLite 负责治理，不负责语义理解。
- Markdown 负责人类审查，不负责大规模召回。
- JSON / logs / object storage 负责可恢复的原始真源。
...
语义索引中的每个 item 都应该能回到可恢复的正文真源。Milvus 可以索引 trace summary、episode、asset 或 codemap chunk，但不应该成为 memory 的唯一存活副本。
...
当前实现已经支持的 anti-faulty-memory 机制包括：

- 原始 trace 留存：trace、episode、activation view、feedback 都是一等持久化对象。
- candidate 缓冲层：任务后总结先进入 `candidate`，而不是直接进入高权重长期层。
- activation provenance：召回结果自带 `source_provenance`、`match_evidence`、`risk_flags`、`llm_use_guidance`。
- feedback 驱动健康度：activation feedback 会刷新 `temperature` 和 `review_status`。
- progressive recall gate：`progressive-recall` 只在新错误、新文件/模块、新阶段或 topic drift 时触发。

当前实现已经比较容易扩展、但还未完整落地的机制包括：

- 更细粒度的作用域标注：module、language、framework、task type、适用条件、已知反例。
- 显式 quarantine / deprecate / validity window 等治理字段。
- 重要资产的 replay / validation 能力。
- conflict detection 与 conflict-aware injection suppression。
- 面向未来 `team` / `organization` 级别资产的更严格晋升规则。
